...
func TestFirewallZonePolicyRoundTrip(t *testing.T) {
	args := FirewallZonePolicyArgs{
		...
		Matching: &FirewallZonePolicyMatchingArgs{
			...
			MatchIpSec:            ptr(true),
			MatchIpSecType:        ptr(FirewallZonePolicyMatchIpSecTypeMatchIpSec),
		},
		Source: FirewallZonePolicySourceArgs{
			ZoneId:             "zone-lan",
			MatchingTarget:     ptr(FirewallZonePolicySourceMatchingTargetNetwork),
			MatchingTargetType: ptr(FirewallZonePolicySourceMatchingTargetTypeObject),
			NetworkIds:         []string{"net-1", "net-2"},
			Port:               ptr("1000-2000"),
			PortMatchingType:   ptr(FirewallZonePolicyPortMatchingTypeSpecific),
			MatchOppositeIps:   ptr(true),
		},
		Destination: FirewallZonePolicyDestinationArgs{
			ZoneId:             "zone-wan",
			MatchingTarget:     ptr(FirewallZonePolicyDestinationMatchingTargetIp),
			MatchingTargetType: ptr(FirewallZonePolicyDestinationMatchingTargetTypeSpecific),
			Ips:                []string{"8.8.8.8"},
			Port:               ptr("443"),
			PortMatchingType:   ptr(FirewallZonePolicyPortMatchingTypeSpecific),
		},
		Schedule: &FirewallZonePolicyScheduleArgs{
			Mode:           ptr(FirewallZonePolicyScheduleModeEveryWeek),
