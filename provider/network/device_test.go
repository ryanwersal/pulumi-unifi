...
func TestDeviceApplyToAndStateRoundTrip(t *testing.T) {
	args := DeviceArgs{
		...
		Switching: &DeviceSwitching{
			...
			PoeMode:     ptr(DevicePoeMode("auto")),
		},
		Led: &DeviceLed{
			Override: ptr(DeviceLedOverride("on")),
			EtherLighting: &DeviceEtherLightingArgs{
				LedMode:    ptr(DeviceLedMode("etherlighting")),
				Mode:       ptr(DeviceEtherLightingMode("speed")),
				Behavior:   ptr(DeviceEtherLightingBehavior("breath")),
				Brightness: ptr(80),
			},
		},
		...
		RadioTable: []DeviceRadioOverride{
			{Radio: "na", Channel: ptr("36"), ChannelWidth: ptr(80), TxPower: ptr("18"), TxPowerMode: ptr(DeviceTxPowerMode("custom")), MinRssi: ptr(-75)},
		},
		...
		PortOverrides: []DevicePortOverride{
			...
			{PortIdx: 1, PoeMode: ptr(DevicePoeMode("off"))},
			// A brand-new override for port 5 with a broad spread of fields.
			{
				PortIdx:                    5,
				Name:                       ptr("Uplink"),
				OpMode:                     ptr(DevicePortOpMode("switch")),
				NativeNetworkId:            ptr("net123"),
				TaggedVlanMgmt:             ptr(DeviceTaggedVlanMgmt("custom")),
				ExcludedNetworkIds:         []string{"netA", "netB"},
				Forward:                    ptr(DevicePortForward("customize")),
				Speed:                      ptr(1000),
				Autoneg:                    ptr(true),
				StormctrlType:              ptr(DeviceStormctrlType("level")),
				...
				Dot1xCtrl:                  ptr(DeviceDot1xCtrl("auto")),
				EgressRateLimitKbps:        ptr(1000),
				EgressRateLimitKbpsEnabled: ptr(true),
				PriorityQueue1Level:        ptr(10),
				FecMode:                    ptr(DeviceFecMode("default")),
				VoiceNetworkId:             ptr("voiceNet"),
				SettingPreference:          ptr(DeviceSettingPreference("manual")),
			},
		},
	}
